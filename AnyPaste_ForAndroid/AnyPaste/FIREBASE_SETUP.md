...
`google-services.json`에 Web OAuth Client가 포함되지 않으면 이메일 로그인만 사용할 수 있습니다. 앱은 이 상태에서 Google 로그인 버튼을 누르면 설정 안내를 표시하며 종료되지 않습니다.
...
Firestore 규칙은 사용자별 문서 접근과 허용 필드를 제한합니다. 클립보드 문서는 생성 시점보다 미래이면서 5분 이내인 `expiresAt`만 허용합니다. 앱은 기본값으로 생성 시점부터 5분을 사용하며, 새 항목이 생성되면 기존 항목은 즉시 삭제합니다. 기존 프로젝트에 규칙과 다른 형태의 문서가 있다면 배포 전에 마이그레이션해야 합니다.
